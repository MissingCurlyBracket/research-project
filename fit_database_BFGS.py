...
def fit_model(sizes, scores, sizes_extrapolation, model_id, rep=5, verbose=True):
    ...
    def objective(beta):  # this returns the residuals of the fit on the training points
        fun = get_fun(beta)
        predictions = fun(sizes)  # Calculate the predicted values using the given sizes
        error = predictions - scores
        objective_value = np.sum(error ** 2)/len(error)  # Calculate the MSE
        return objective_value

    # Define the gradient of your objective function
    def objective_der(beta, epsilon = 1e-8):
        ...
        for i in range(len(beta)):
            ...
            grad[i] = (objective(beta_plus) - objective(beta_minus)) / (2 * epsilon)
        return grad

    def get_gradient(beta):
        gradient = grad(get_fun(beta))
        return gradient
    ...
    while i <= rep:  # try repeatedly to fit a model
        ...
        # start fitting
        beta = sp.optimize.minimize(objective, init, method="BFGS", jac=objective_der).x
